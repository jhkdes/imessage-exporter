...
use imessage_database::{
    ...
    util::{
        dates::{format, format_utc, get_local_time, readable_diff, TIMESTAMP_FACTOR},
        plist::parse_plist,
    },
};
    ...
    fn iter_messages(&mut self) -> Result<(), RuntimeError> {
        ...
        for message in messages {
            ...
            // Message replies and reactions are rendered in context, so no need to render them separately
                ...
                JSON::write_to_file(self.get_or_create_file(&msg), &",");
            }
            current_message += 1;
...
impl<'a> Writer<'a> for JSON<'a> {
    fn format_message(&self, message: &Message, indent_size: usize) -> Result<String, TableError> {
        ...
        // Open a curly bracket to start a JSON element
        self.add_line(&mut formatted_message, &"{", &indent);

        // Add message ID
        self.add_line(&mut formatted_message, &format!("\"msg_id\": \"{}\",", message.guid), &indent);

        // Add message date in UTC timestamp
        self.add_line(&mut formatted_message, &format!("\"sent_time\": {},", self.get_time_utc(message)), &indent);
        ...
        self.add_line(
            ...
            &format!("\"sender\": \"{}\",", self.config.who(message.handle_id, message.is_from_me)),
            &indent,
        );
        ...
        if message.is_deleted() {
            self.add_line(
                &mut formatted_message,
                &format!("\"action\": \"{}\",", "This message was deleted from the conversation!"),
                &indent,
            );
        }
        ...
        // Open a curly bracket to start a JSON element
        self.add_line(&mut formatted_message, &"\"raw_data\": \"\" }", &indent);
...
impl<'a> JSON<'a> {
    ...
    fn get_time_utc(&self, message: &Message) -> String {
        let mut date = format_utc(&message.date(&self.config.offset));
        let read_after = message.time_until_read(&self.config.offset);
        if let Some(time) = read_after {
            if !time.is_empty() {
                let who = if message.is_from_me {
...
mod balloon_format_tests {
    ...
    use crate::{exporters::exporter::BalloonFormatter, Config, Exporter, JSON};
